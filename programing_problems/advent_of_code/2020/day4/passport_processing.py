...
like data from North Pole Credentials, not a passport at all! Surely,
...
valid values. Continue to treat cid as optional. In your batch file, how many passports are valid?
...
def is_valid_hgt(hgt: str) -> bool:
    try:
        return (hgt[-2:] == 'cm' and 150 <= int(hgt[0:-2]) <= 193) or \
               (hgt[-2:] == 'in' and 59 <= int(hgt[0:-2]) <= 76)
    except ValueError:
        pass
...
def get_num_of_valids_passport(passports: List[dict]) -> int:
    valid_cnt = 0
    for passport in passports:
        if (
            all(key in passport.keys() for key in ('eyr', 'hgt', 'pid', 'ecl', 'byr', 'hcl', 'iyr')) and
...
if __name__ == "__main__":
    ...
    with open(filepath) as f:
        passports_data = f.read().strip().split('\n\n')
        for passport_data in passports_data:
            passport_dict = {}
            for key_value in passport_data.split():
                key, value = key_value.split(':')
                passport_dict[key] = value
            passports.append(passport_dict)

    print(get_num_of_valids_passport(passports))
